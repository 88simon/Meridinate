...
- If a wallet is a Sniper Bot, it inflates quality metrics — do not trust raw win rate

CLASSIFICATION BUCKETS — every major finding MUST end in one of these:
- ALLOWLIST CANDIDATE: wallet is trustworthy enough to use as anti-rug confirmation signal
- DENYLIST CANDIDATE: wallet/cluster is adversarial or part of toxic flow — filter it out
...
After your prose report, you MUST output a JSON block fenced with ```json ... ``` containing:
{
  ...
  "denylist_candidates": [{"address": "full_wallet_address", "type": "toxic_flow|sybil|rug_deployer|sniper_bot|high_rug_exposure", "confidence": "high|medium|low", "reason": "one sentence", "supporting_signals": ["signal1: value", "signal2: value"]}],
  ...
  "recommended_actions": [
    {
      "action_type": "add_bot_denylist_wallet|add_bot_allowlist_wallet|add_watch_wallet|remove_bot_allowlist_wallet|remove_bot_denylist_wallet|remove_watch_wallet|add_intel_tag|add_nametag|queue_wallet_pnl_refresh|queue_wallet_funding_refresh",
...
RECOMMENDED ACTIONS:
For every ALLOWLIST CANDIDATE with confidence high or medium, emit: add_bot_allowlist_wallet
For every DENYLIST CANDIDATE with confidence high or medium, emit: add_bot_denylist_wallet
For every WATCH-ONLY wallet, emit: add_watch_wallet
...
def run_intel_report(
    ...
    if housekeeper_structured:
        ...
        # Fallback: pass full prose if no structured data available
