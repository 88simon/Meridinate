...
export function SwabTab({ isActive }: SwabTabProps) {
  ...
  const [positions, setPositions] = useState<SwabPositionsResponse | null>(
    null
  );
  ...
  const walletGroups = useMemo((): WalletGroup[] => {
    ...
    Array.from(grouped.entries()).forEach(
      ([wallet_address, walletPositions]) => {
        const holding = walletPositions.filter(
          (p: SwabPosition) => p.still_holding
        ).length;
        const sold = walletPositions.filter(
          (p: SwabPosition) => !p.still_holding
        ).length;
        ...
        const avg_pnl =
          pnlValues.length > 0
            ? pnlValues.reduce((a: number, b: number) => a + b, 0) /
              pnlValues.length
            : null;
  ...
  const handleTriggerPnlUpdate = async () => {
    ...
    try {
      ...
      toast.success(
        `Updated PnL for ${result.positions_updated} positions across ${result.tokens_updated} tokens`
      );
      fetchData();
    } catch {
  ...
  const handleBatchUntrack = async () => {
    ...
    try {
      const result = await batchStopSwabPositions(
  ...
  const toggleAllSelection = () => {
    ...
    const trackablePositions = positions.positions.filter(
      (p) => p.still_holding && p.tracking_enabled
    );
  ...
  const formatPnl = (
    ...
    const colorClass =
      pnl > 1 ? 'text-green-500' : pnl < 1 ? 'text-red-500' : 'text-yellow-500';
  ...
  const trackablePositions =
    positions?.positions.filter((p) => p.still_holding && p.tracking_enabled) ||
    [];
  const allSelected =
    trackablePositions.length > 0 &&
    selectedPositions.size === trackablePositions.length;
      ...
      <div className='flex items-center justify-center gap-2 py-1'>
        {settings && (
          <button
            ...
            className='hover:bg-muted flex items-center gap-1.5 rounded px-2 py-1 transition-colors'
            title={
              settings.auto_check_enabled
                ? 'Click to disable auto-check'
                : 'Click to enable auto-check'
            }
          >
            <div
            ...
            />
            <span
              className={`text-xs font-medium ${settings.auto_check_enabled ? 'text-green-500' : 'text-muted-foreground'}`}
            >
            ...
            {schedulerStatus?.next_check_at && settings.auto_check_enabled && (
              <span className='text-muted-foreground text-xs'>
                (
                {new Date(schedulerStatus.next_check_at).toLocaleTimeString(
                  [],
        ...
        >
          <RefreshCw
            className={`mr-1 h-3 w-3 ${pnlUpdateLoading ? 'animate-spin' : ''}`}
          />
          PnL
        </Button>
        ...
        >
          ...
          {checkLoading
            ? '...'
            : `Check (~${stats?.estimated_check_credits ?? 0})`}
        </Button>
      ...
      <div className='rounded-lg border'>
        <div className='border-b px-3 py-2'>
          <div className='flex items-center justify-between'>
            ...
            <span className='text-muted-foreground text-xs'>
              {positions?.total ?? 0} positions · {stats?.unique_wallets ?? 0}{' '}
              wallets
            </span>
          </div>
        </div>
              ...
              <thead className='bg-card sticky top-0 z-10'>
                <tr className='text-muted-foreground border-b text-left text-xs'>
                  ...
                  <th className='w-20 px-3 py-1.5'>
                    <Button
                      variant={
                        selectedPositions.size > 0 ? 'destructive' : 'ghost'
                      }
                      size='sm'
                      ...
                      disabled={
                        batchUntrackLoading || trackablePositions.length === 0
                      }
                    >
                      {batchUntrackLoading ? (
                  ...
                  group.positions.map((pos, posIndex) => (
                    ...
                    >
                          ...
                          <div className='flex flex-col gap-0.5'>
                            ...
                            {group.total_positions > 1 && (
                              <span className='text-muted-foreground text-[10px]'>
                                {group.holding_count}H / {group.sold_count}S ·{' '}
                                {group.avg_pnl?.toFixed(1) ?? '--'}x avg
                              </span>
                            )}
                            <WalletTagLabels
                      ...
                      <td className='px-3 py-1 text-right font-mono'>
                        {formatMarketCap(pos.entry_market_cap)}
                      </td>
                      <td className='px-3 py-1 text-right font-mono'>
                        {pos.still_holding
                          ? formatMarketCap(pos.current_market_cap)
                          : formatMarketCap(pos.exit_market_cap)}
                      </td>
                      <td className='px-3 py-1'>
                        {pos.still_holding ? (
                      ...
                      <td className='px-3 py-1 text-right font-mono'>
                        {pos.pnl_ratio != null ? (
                          ...
                          <span
                            ...
                            title='Estimated from market cap (no transaction data)'
                          >
                      ...
                      <td className='text-muted-foreground px-3 py-1 text-right'>
                        {formatHoldTime(pos.hold_time_seconds)}
                      </td>
                      <td className='text-muted-foreground px-3 py-1'>
                        {formatLastUpdated(pos.position_checked_at)}
                      </td>
        ...
        {positions && positions.total > pageSize && (
          <div className='flex items-center justify-between border-t px-3 py-2'>
            <span className='text-muted-foreground text-xs'>
              {page * pageSize + 1}-
              {Math.min((page + 1) * pageSize, positions.total)} of{' '}
              {positions.total}
            </span>
